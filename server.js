...
const ALLOWED_ASSET_TYPES = /* @__PURE__ */ new Map([
  ["image/png", "png"],
  ["image/jpeg", "jpg"],
  ["image/webp", "webp"],
  ["image/gif", "gif"],
...
function publicDocumentPath(documentType, slug) {
...
function createDefaultCard(slug) {
...
function isValidSessionToken(token) {
  ...
  try {
    ...
    return Number.isFinite(session.issuedAt) && Number(session.issuedAt) <= Date.now() && Date.now() - Number(session.issuedAt) < SESSION_TTL_MS;
  } catch {
    return false;
  }
}
...
function uniqueSlug(cards, requestedSlug, ignoredId = "") {
...
async function readSiteData() {
  if (hasBlobStorage()) {
    ...
    if (blob?.statusCode === 200) {
      const raw2 = await streamToString(blob.stream);
...
function normalizeSiteData(value) {
  ...
  return {
    ...
    cards: candidate.cards.map((card, index) => {
      const slug = uniqueSlug(candidate.cards.slice(0, index), card.slug || card.title || `card-${index + 1}`);
      return {
        ...card,
...
function createPublicSitePayload(siteData, requestedSlug = "") {
  const slug = sanitizeSlug(requestedSlug);
  const cards = slug ? siteData.cards.filter((card) => card.isVisible && card.slug === slug) : [];
  return {
    hub: siteData.hub,
    cards
  };
...
function createPublicCardIndex(siteData) {
  return {
    cards: siteData.cards.filter((card) => card.isVisible).map((card) => ({
      id: card.id,
      slug: card.slug,
      title: card.title,
